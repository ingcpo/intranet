...
$new_url = 'http://srviisbog17:81/Produccion/LogOnForm.aspx?ReturnUrl=%2fProduccion';
...
if ($new_content !== $content) {
    ...
    if ($stmt->execute()) {
        echo "Successfully updated Torre 5 Urgencias link to srviisbog17.";
    } else {
        echo "Error updating database: " . $stmt->error;
    }
} else {
    echo "No changes made. Link might already be correct or pattern did not match.";
    if (strpos($content, $new_url) !== false && preg_match('/Torre 5(?:<br>|\s+)Urgencias/i', $content)) {
        echo " Link appears to be already pointing to srviisbog17.";
    } else {
